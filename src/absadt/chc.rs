...
impl AbsClause {
    ...
    fn rescale_idx_map(vars: &VarInfos) -> VarHMap<Term> {
        ...
        for (supposed_idx, var) in vars.iter().enumerate() {
            ret.insert(var.idx, term::var(supposed_idx, var.typ.clone()));
        }
        ret
    }
                    ...
                    match term.get() {
                        ...
                        _ => {
                            return Err(Error::from_kind(ErrorKind::Msg(
                                "In the right-hand side some argument is not a variable"
                                    .to_string(),
                            )));
                        }
                    }
    ...
    if !unresolved_roots.is_empty() {
        ...
        if roots.is_empty() {
            ...
            for (_, children) in &unresolved_roots {
                ...
                let ok = children
                    .iter()
                    .all(|c| reaches_well_formed(*c, &roots_set, &unresolved_map, &mut visited));
                if !ok {
                    bail!("hyper resolution is ill-structured")
                }
            }
...
impl<'a> AbsInstance<'a> {
    ...
    pub fn check_sat(&self) -> Res<either::Either<(), CallTree>> {
        ...
        let (b, eldarica_error) = super::chc_solver::portfolio(self)?
            .either(|_| (true, false), |(_, eld_err)| (false, eld_err));
        if b {
